...
Tn<-18.5 #end of simulations in year
steps<-18.5*12  #Number of steps
dt<-Tn/steps  # size of steps

### Swap contract 1 ###

start1<-0 #Start of swap contract 1
end1<-18.5 #end of swap contract 2
...
start2<-0 # start of swap agreement 2
end2<-18 #end of swap contract 2
...
#   geom_hline(aes(yintercept = 0.043,linetype="dashed"), color="red")+
...
Value1<-PV_IRS_Payer(start1,end1, tenor, steps,n, dt, N1, a, sigma, fixed1,  r, ZCB,  eps)
Value2<-PV_IRS_Payer(start2,end2, tenor, steps-6,n, dt, N2, a, sigma, fixed2,  r, ZCB,  eps)
...
####### Netting values plotted ######

mean_netting<-rep(NA,steps+1)
...
plot_IRS_Values_netting <- ggplot(data_plot_netting, aes(time, value)) +
  geom_line(aes(colour = variable)) +
  theme(legend.position = "none")+
  ggtitle("Interest Rate Swap Portfolio Value 2019")+xlab("Time in years")+
  scale_y_continuous(name="IRS value", labels = scales::comma)+
  theme(plot.title = element_text(hjust = 0.5))

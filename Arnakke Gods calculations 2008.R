...
zero_rates<-read.csv2('Interest rate data 2008.csv', header = T)  
CDS_Crosss<-read.csv2("ITRAXX Crossover 2008.csv", header=T)
CDS_Europe<-read.csv2("ITRAXX europe 2008.csv", header=T)
...
Tn<-30 #end of simulations in year
steps<-30*12  #Number of steps
dt<-Tn/steps  # size of steps

### Swap contract ###

start<-0 #Start of swap contract
end<-30  #end of swap contract
...
plot_rate <- ggplot(data_plot, aes(time, value)) +
  ...
  ggtitle("Hull-White interest rate paths 2008 ")+xlab("Time in years")+ylab("Interest rate ")+
...
### calculate interest swap values ###
...
plot_IRS_Values <- ggplot(data_plot, aes(time, value)) +
  ...
  theme(legend.position = "none")+
  ggtitle("Interest rate swap value 2008")+xlab("Time in years")+
...
plot(time_vector,EDE_v, type = "l")
lines(time_vector,EDE_v)
...
ggplot(data_negative_exposure,aes(time,value))+
  ...
  xlab("Time in years")+
  ggtitle("Negative Exposure 2008")+
  theme(plot.title = element_text(hjust = 0.5))

#### CVA calculations ###

##Makes vector for CVA and DVA values
